...
contract IdentityRegistryStorage is IIdentityRegistryStorage, IRSStorage {
     ...
     */
    function modifyStoredIdentity(address _userAddress, IIdentity _identity) external override{
        require(
            _userAddress != address(0)
            && address(_identity) != address(0)
        , "invalid argument - zero address");
