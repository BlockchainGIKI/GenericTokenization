...
contract Token is Ownable {
    ...
    if (redemptionState != Redemption.REDEEMABLE && redemptionState != Redemption.REDEEMABLE_EXTENDIBLE && redemptionState != Redemption.REDEEMABLE_EXCHANGEABLE_EXTENDIBLE){
    ...
    // Functions /////////
    ///////////////////
    constructor(string memory _name, string memory _symbol, uint256 _initialSupply, Redemption _redemptionState, uint256 _buybackDate, address _paymentToken, uint256 _price){
    ...
    function issueToken(uint256 _amount, address _to) public onlyOwner isNonZero(_amount){
        asset.transfer(_to, _amount);     
    }

    function exchangeToken(address _exchangeToken, uint256 _amount) public isExchangeable isAllowedExchangeToken(_exchangeToken) hasSufficientBalance(_amount, msg.sender) hasSufficientAllowance(_amount, msg.sender) {
    ...
    function redeemToken(uint256 _amount) public isRedeemable isNonZero(_amount) hasSufficientBalance(_amount, msg.sender) hasSufficientAllowance(_amount, msg.sender) {
        require(buybackDate < block.timestamp, 'Buyback date should be less than current time');
        require(price*_amount <= IERC20(paymentToken).balanceOf(address(this)),'The smart contract does not have sufficient funds');
        asset.transferFrom(msg.sender, address(this), _amount);
        bool success = IERC20(paymentToken).transfer(msg.sender, price*_amount);
        if (!success) {
            revert Token__ExchangeFailed();
        }
    }
